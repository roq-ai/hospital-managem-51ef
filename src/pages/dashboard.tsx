...
import { getNewTabelHS } from 'apiSdk/new-tabel-hs';
...
import { NewTabelHListPage as ShowCaseList } from 'pages/new-tabel-hs';

function DashboardPage() {
  const { session } = useSession();

  const newTabelHStat = useSWR<PaginatedInterface<NewTabelHInterface>>(
    () => `/new-tabel-hs`,
    () => getNewTabelHS({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'New Tabel HS',
      result: newTabelHStat?.data?.totalCount,
      isLoading: newTabelHStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`new-tabel-hs`} passHref legacyBehavior>
                  <Button
